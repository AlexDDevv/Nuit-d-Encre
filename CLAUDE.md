...
- **XP une seule fois** : `grantXpService` exige un `xpKey` (helpers `utils/xp-keys.ts`) et n'accorde rien si `(user, type, xpKey)` est déjà en base (index unique partiel). Utiliser une clé **stable** (id du livre, ISBN, id auteur, `review+voter`), jamais l'id d'une entité supprimable/recréable — sinon annuler/refaire une action refait gagner de l'XP.
