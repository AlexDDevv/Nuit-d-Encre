        ...
        <div className="order-1 flex flex-col gap-5 lg:order-2 lg:sticky lg:top-24">
            <div>
                <div className="mb-3 flex items-center gap-2.5">
                    <Ornament width="w-5" />
                    <span className="font-body text-xs font-bold uppercase tracking-[0.24em] text-muted-foreground">
                        Aperçu en direct
                    </span>
                </div>
                <div className="rounded-2xl border-2 border-border bg-background/60 p-4 shadow-[inset_0_1px_0_hsl(43_30%_60%/0.05)]">
                    {previewDismissed ? (
                        <div className="flex flex-col items-center gap-3 px-4 py-8 text-center">
                            <p className="font-quote text-base italic text-muted-foreground/70">
                                La bannière a été fermée par l'utilisateur.
                            </p>
                            <button
                                type="button"
                                onClick={onReplay}
                                className="inline-flex items-center gap-1.5 rounded-md border-2 border-border px-3 py-1.5 font-body text-xs font-bold text-muted-foreground transition-colors hover:border-primary/55 hover:text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
                            >
                ...
                {draft.hasAction && draft.actionTarget && (
                    <div className="mt-3 flex items-center gap-2 rounded-lg border border-border bg-popover/40 px-3 py-2 font-body text-xs text-muted-foreground">
                        <LuExternalLink
                            size={13}
            ...
            <div className="rounded-xl border-2 border-border bg-card">
                <div className="flex items-center gap-2.5 border-b-2 border-border/70 px-4 py-3">
                    ...
                    <h3 className="font-body text-xs font-bold uppercase tracking-[0.16em] text-foreground/85">
                        Bannières enregistrées
                    </h3>
                    <span className="ml-auto rounded-full bg-muted/60 px-2 py-0.5 font-body text-xs font-bold text-muted-foreground">
                        {saved.length}
                    </span>
                </div>
